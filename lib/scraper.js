...
function extractPagination($, currentPage, baseUrl = null) {
  ...
  // Method 1: .hpage (samehadaku.li theme) - a.l = prev, a.r = next
  const hpage = $('.hpage');
  if (hpage.length) {
...
// ==================== BROWSER FETCH (Puppeteer) ====================
// For pages where server/download data is loaded via JavaScript
...
async function getBrowser() {
  ...
  try {
    ...
    // Local dev fallback — try system Chrome
    const os = require('os');
    const paths = [
  ...
  _browserLastUsed = Date.now();
  console.log('[BROWSER] Launched headless browser');
  
  // Auto-close idle browser
  const checkIdle = setInterval(() => {
    if (_browser && Date.now() - _browserLastUsed > BROWSER_IDLE_TIMEOUT) {
      _browser.close().catch(() => {});
      _browser = null;
      clearInterval(checkIdle);
      console.log('[BROWSER] Closed idle browser');
    }
  }, 15000);
...
/**
 * Fetch page using headless browser (for JS-rendered content)
 * Returns the fully rendered HTML after JavaScript execution
 * @param {string} url - URL to fetch
 * @param {object} options - { waitFor: selector/ms, timeout: ms }
 */
async function fetchPageWithBrowser(url, options = {}) {
  const cacheKey = `browser:${url}`;
  const cached = cache.get(cacheKey);
  if (cached) {
    console.log(`[BROWSER CACHE HIT] ${url}`);
    return cached;
  }
  
  const { waitFor = 3000, timeout = 20000 } = options;
  const browser = await getBrowser();
  const page = await browser.newPage();
  
  try {
    // Set realistic browser fingerprint
    await page.setUserAgent(getSessionUA());
    ...
    console.log(`[BROWSER] Got rendered HTML: ${html.length} bytes`);
