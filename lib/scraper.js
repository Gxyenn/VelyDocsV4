...
function extractPagination($, currentPage, baseUrl = null) {
  ...
  // Try to extract total items from page text (e.g., "Showing 1-20 of 150 results")
  ...
  // Also try to get total from WordPress-style "Page X of Y" text
  ...
  if (hpage.length) {
    ...
    // Scan semua link di .hpage untuk extract page numbers
    hpage.find('a').each((i, el) => {
      ...
      // Juga cek text content untuk angka halaman
      const text = $(el).text().trim();
      const num = parseInt(text);
      if (!isNaN(num) && num > totalPages) totalPages = num;
    });

    // Juga cek elemen non-link (span, current page) di .hpage
    hpage.find('span, strong, em').each((i, el) => {
      const text = $(el).text().trim();
      const num = parseInt(text);
      if (!isNaN(num) && num > totalPages) totalPages = num;
    });
  ...
  if (pageNumbers.length) {
    pageNumbers.each((i, el) => {
      ...
      if (!isNaN(num) && num > totalPages) totalPages = num;
      // Cek href juga untuk deteksi totalPages lebih akurat
      const href = $(el).attr('href') || '';
      const m = href.match(/page[=/](\d+)/);
      if (m) {
        const hrefNum = parseInt(m[1]);
  ...
  // Method 3: Scan semua link dengan page parameter
  ...
  // Method 4: Cek link "Last" / "Terakhir" / ">>" / "»"
  $('a').each((i, el) => {
  ...
  // Method 5: Dots/ellipsis pagination - cek elemen setelah dots
  ...
  // Fallback: deteksi next/prev jika belum terdeteksi
  ...
  // Pastikan totalPages minimal = current page
  if (totalPages < page) totalPages = page;

  const hasPrevPage = page > 1;
  const hasNextPage = hasNext || page < totalPages;

  // Estimate totalItems if not already found
  // Count items on current page and multiply by total pages
  if (totalItems === null) {
    const itemCount = $('.listupd .bs, .bsx, .post-item, .soralist ul li').length;
    if (itemCount > 0 && totalPages > 0) {
      // For the last page, the actual count might be less
      // This is an estimate
